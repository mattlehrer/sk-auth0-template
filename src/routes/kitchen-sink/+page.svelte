...
<main>
	<section id="headings">
		...
		<footer>
			<p>
				See the <a target="_blank" href="https://www.w3.org/TR/html5/dom.html#heading-content"
					>Heading Content spec.</a
				>
			</p>
			<p>Note: these two paragraphs are contained in a <code>footer</code> element.</p>
		</footer>
	</section>
	<hr />
	<section id="sections">
		...
		<article>
			<p>
				This paragraph is nested inside an <code>article</code> element. It contains many different,
				sometimes useful, <a href="http://www.w3schools.com/tags/">HTML5 elements</a>. Of course
				there are classics like <em>emphasis</em>, <strong>strong</strong>, and <small>small</small>
				but there are many others as well. Hover the following text for abbreviation element:
				<abbr title="abbreviation">abbr</abbr>. You can define <del>deleted text</del> which often
				gets replaced with <ins>inserted</ins> text.
			</p>
			<p>
				You can also use <kbd>keyboard text</kbd>, which sometimes is styled similarly to the
				<code>&lt;code&gt;</code>
				or <samp>samp</samp> elements. Even more specifically, there is an element just for
				<var>variables</var>. Not to be mistaken with block quotes below, the quote element lets you
				denote something as <q>quoted text</q>. Lastly don't forget the sub (H<sub>2</sub>O) and sup
				(E = MC<sup>2</sup>) elements.
			</p>
			<section>
				<p>
					This paragraph is contained in a <code>section</code> element of its parent
					<code>article</code> element.
				</p>
				<p>
					↓ The following paragraph has the <code>hidden</code> attribute and should not be displayed.
					↓
				</p>
		...
		<footer>
			<p>
				See the <a target="_blank" href="https://www.w3.org/TR/html5/dom.html#sectioning-content"
	...
	<section id="phrasing">
		...
		<p>
			<code>bdi</code>: Some languages read right to left, <bdi lang="ar">مرحبا</bdi>.
			<code>bdo</code>: <bdo dir="rtl">The normal direction has been overridden.</bdo>
		</p>
		<p>
			<code>em</code> is used for <em>emphasis</em> and usually renders as italics, contrast that
			with <code>i</code> which is used for alternate voice or to offset from the normal (such as a
			phrase from a different language or taxonomic designation): <i>E. coli</i> can be bad.
			<code>strong</code>
			is used for <strong>importance or urgency</strong> and usually renders as bold, contrast that
		...
		<p>
			<code>data</code> can be used to specify <data value="2018-09-24T05:00-07:00">5 A.M.</data>
			that is machine-readable, but <code>time</code> is a better choice for specifying
			<time datetime="2018-09-24T05:00-07:00">5 A.M.</time> in a machine-readable format.
		</p>
		<p>
			<code>del</code> can be <del datetime="2017-10-11T01:25-07:00">varily</del> used to mark
			deletions. <code>ins</code> marks <ins datetime="2007-12-19 00:00Z">insertions</ins>.
			<code>s</code>: similar to <code>del</code>, but used to mark content that is no longer
			relevant. <s>Windows XP version available.</s> <code>u</code>: a holdover with no real meaning
		...
		<p>
			...
			<dfn lang="fr" title="French: indefinable quality">je ne sais quoi</dfn> to one's prose.
		</p>
		<p>
			<code>q</code>: The W3C page <cite>About W3C</cite> says the W3C’s mission is
			<q cite="https://www.w3.org/Consortium/"
				...
				that ensure long-term growth for the Web</q
			>.
		</p>
		...
		<p>
			<code>var</code>: To log in, type <kbd>ssh <var>user</var>@example.com</kbd>, where
		...
		<p>
			<code>sub</code> is used for subscripts: H<sub>2</sub>O. <code>sup</code> is used for
			superscripts: E = MC<sup>2</sup>. <code>small</code> is used for side comments:
			<q>I wrote this whole document. <small>[Editor's note: no he did not]</small></q>
			<code>wbr</code>: used to specify where a word may break and it is super<wbr />cali<wbr
			/>fragil<wbr />istic<wbr />expiali<wbr />do<wbr />cious.
		</p>
		<footer>
			<p>
				See the <a target="_blank" href="https://www.w3.org/TR/html5/dom.html#phrasing-content"
	...
	<section id="palpable">
		...
		<blockquote>
			<p>
				This is a mult-line blockquote with a cite reference. People think focus means saying yes to
				the thing you’ve got to focus on. But that’s not what it means at all. It means saying no to
				the hundred other good ideas that there are. You have to pick carefully. I’m actually as
				proud of the things we haven’t done as the things I have done. Innovation is saying no to
				1,000 things.
			</p>
		...
		<figure>
			<img src="https://picsum.photos/402/295" alt="lorem picsum" />
			<figcaption>
				Figure 1: A Lorem Picsum from <a href="https://picsum.photos/">picsum.photos</a>
			</figcaption>
		</figure>
		<br /><br />
...
&#gt;</samp
			...
			<tbody>
				...
				<tr>
					...
					<td
						...
						id nibh ultricies vehicula ut id elit.</td
					>
				</tr>
				<tr>
					<td>Last One</td>
					...
					<td
						...
						purus sit amet fermentum.</td
					>
				</tr>
			</tbody>
		</table>
		<p id="table-summary">
			In the following table, characteristics are given in the second column, with the negative side
		...
		<table>
			<caption
				>Complex table with a <code>thead</code>, multiple <code>tbody</code> elements, and a
				<code>tfoot</code>.</caption
			>
			<thead>
				<tr>
					<th></th>
		...
		<footer>
			<p>
				See the <a target="_blank" href="https://www.w3.org/TR/html5/dom.html#palpable-content"
...
is a sample content.</p>"
		...
		<p><code>img</code>: <img src="https://picsum.photos/150/150" alt="Bill Murray" /></p>
		<p><code>math</code>:</p>
		...
		<p>
			...
			<picture>
				<source
					srcset="https://picsum.photos/240/300 2x,
https://picsum.photos/120/150 1x"
				/>
				<img src="https://picsum.photos/120/150" alt="Lorem Picsum" />
			</picture>
		</p>
		<p>
			<code>svg</code>:
...
7.184 15.909 15.91z"
		...
		<footer>
			<p>
				See the <a target="_blank" href="https://www.w3.org/TR/html5/dom.html#embedded-content"
